...
async function main() {
  const { MATERIALS_NPM_REGISTRY = '', MATERIALS_UNPKG } = env
  ...
  const command = `pnpm publish --registry ${MATERIALS_NPM_REGISTRY}`
  ...
  // 发布根目录
  const rootPath = r('../')
  const { name, version } = await getPkgJSON(rootPath)
  const isExit = await checkVersionExit(name, version, MATERIALS_NPM_REGISTRY)
  if (isExit) {
    console.log(`❌ ${name}@${version} has been published, skip`)
    return
  ...
  console.log(`🎉 Unpkg: ${MATERIALS_UNPKG}/${name}@latest/build/materials.json`)
